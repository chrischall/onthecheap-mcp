...
describe('cotc_search_posts', () => {
  it('maps arguments onto the client and returns compact summaries', async () => {
    ...
    const out = parse(
      await h.callTool('cotc_search_posts', {
        query: 'free museum',
        category: 13,
        location: 6276,
        after: '2026-01-01',
  ...
  it('returns full records when compact is disabled', async () => {
    ...
    const out = parse(await h.callTool('cotc_search_posts', { compact: false }));
  ...
  it('rejects a non-ISO date instead of passing it through', async () => {
    const h = await setup();
    const result: any = await h.callTool('cotc_search_posts', { after: '07/25/2026' });
...
describe('cotc_get_post', () => {
  it('returns the body as readable text by default', async () => {
    const h = await setup();
    getPost.mockResolvedValue(POST);
    const out = parse(await h.callTool('cotc_get_post', { post: 'free-museum' }));
  ...
  it('returns raw markup when asked for html', async () => {
    const h = await setup();
    getPost.mockResolvedValue(POST);
    const out = parse(await h.callTool('cotc_get_post', { post: '1', format: 'html' }));
    expect(out.content).toContain('<b>10am</b>');
  });
});

describe('cotc_list_events', () => {
  const DAY = {
    date: '2026-07-25',
    events: [
  ...
  it('lists a given day with free counts', async () => {
    ...
    const out = parse(await h.callTool('cotc_list_events', { date: '2026-07-25' }));
  ...
  it('filters to free listings while still reporting the day total', async () => {
    const h = await setup();
    getEventsForDate.mockResolvedValue(DAY);
    const out = parse(await h.callTool('cotc_list_events', { free_only: true, date: '2026-07-25' }));
  ...
  it('defaults to today when no date is given', async () => {
    const h = await setup();
    getEventsForDate.mockResolvedValue(DAY);
    await h.callTool('cotc_list_events', {});
    expect(getEventsForDate.mock.calls[0][0]).toMatch(/^\d{4}-\d{2}-\d{2}$/);
  });
});

describe('cotc_events_month_overview', () => {
  it('reports true totals rather than the truncated preview counts', async () => {
    ...
    const out = parse(await h.callTool('cotc_events_month_overview', { month: '2026-08' }));
...
describe('taxonomy and health tools', () => {
  it('lists categories with decoded names', async () => {
    ...
    const out = parse(await h.callTool('cotc_list_categories', {}));
  ...
  it('lists locations', async () => {
    ...
    const out = parse(await h.callTool('cotc_list_locations', {}));
  ...
  it('reports health', async () => {
    ...
    expect(parse(await h.callTool('cotc_healthcheck', {})).ok).toBe(true);
  });
});
